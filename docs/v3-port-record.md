...
**We already load rungs.** Our geometry is entirely header-driven, so a rung is
just a v3 container with different numbers in it. `v3_rung_containers` asserts
this across all 19 depths against `tests/v3_ladder_rungs.json`, which is
generated by running upstream's own `ladder_layer_indices` — deriving the
subsets ourselves would risk agreeing with our own bug. No loader change was
needed. The KV cache falls out of `num_layers` and scales with depth.
