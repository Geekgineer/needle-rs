...
pub fn decoder_layer_forward(
    ...
    zc_rms_norm_vec(&mut normed, &layer.self_attn_norm);
    ...
    zc_rms_norm_vec(&mut normed2, &layer.cross_attn_norm);

    cross_attn_incremental(&normed2, tmp, &layer.cross_attn, cfg, enc_kv);
    ...
    if let (Some(ffn), Some(ffn_norm)) = (&layer.ffn, &layer.ffn_norm) {
        let d_ff = ffn.w1.out_feat;
        let mut normed3 = x.clone();
        zc_rms_norm_vec(&mut normed3, ffn_norm);
        tmp.resize(d, 0.0);
        ffn_forward(&normed3, tmp, ffn, d_ff);
        let ffn_gate = sigmoid(layer.ffn_gate);
        for (xi, &ti) in x.iter_mut().zip(tmp.iter()) {
            *xi += ffn_gate * ti;
        }
